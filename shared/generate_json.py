...
def main():
    ...
    for idx, f in enumerate(files):
        ...
        if dt:
            ...
            status   = f"  ✅  {f.name}  →  {date_str} {time_str}"
        else:
            time_str = '00:00'
            hour_str = '00h'
            date_str = None
        ...
        results.append({
            ...
            "hour":      hour_str,
            "date":      date_str,
            "timestamp": dt,  # Pour le tri par date/heure complète
        })

    # Trie par date/heure complète croissante
    results.sort(key=lambda p: p['timestamp'] if p['timestamp'] else datetime.min)

    # Réassigne les IDs / sortIndex après tri et supprime le champ timestamp (non sérialisable)
    for i, p in enumerate(results):
        p['id'] = i
        p['sortIndex'] = i  # Ordre chronologique réel (date + heure), pour le tri dans l'app
        del p['timestamp']
